...
describe("GET api/session/:UID", () => {
  test.only("should return a 200 status and all sessions with specified UID", () => {
    return request(app)
      .get("/api/session/5zXz4PmHbG83R2PAC6CJY6sVSNe7")
      .expect(200)
      .then((res) => {
        const resBodySessions = res.body.sessions;
        console.log(resBodySessions);
        expect(resBodySessions.length).toBe(6);
        resBodySessions.forEach((session) => {
...
describe("/api/session error handling", () => {
  test("should return 404 ", () => {
    return request(app)
      .get("/api/session/rW1i3xN8mGbP5FJ2Y9U6vQz4cHs8")
      .expect(404)
      .then((res) => {
        const resBodyMsg = res.body.msg;
        expect(resBodyMsg).toBe("user not found");
